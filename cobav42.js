// silent-scanner-v4.2.js - Silent Endpoint & Secrets Scanner (bookmarklet) - UPDATED v4.2
// - Aggressive fetch options for .js/.chunk.js/.json/.xml to improve recall
// - Whitelist extension: .html, .htm, .js, .json, .xml
// - Modes: smart (mime-aware), ext-only, aggressive (force fetch local text-like resources)
// - Concurrency: 5, MAX_DEPTH: 3, same-origin fetch only, record external endpoints but don't deep-scan
// - Preserve UI from v3, export JSON manual (result-v4.2.json)
// - Detect: API (firebase/google client), fuzzing endpoints, sensitive info, misconfig (.env, .git, .htpasswd), etc.
...
(async function(){
  ...
  Object.assign(panel.style, {
    ...
    maxHeight: '76vh', overflowY: 'auto', backgroundColor: '#fff', color: '#111',
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px">
    <strong style="font-size:14px">Silent Scanner (v4.2)</strong>
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
    <div style="display:flex;gap:6px;align-items:center">
      ...
      <select id="scan-mode" title="Scan mode" style="padding:4px;">
        ...
        <option value="ext-only">Extension only</option>
        <option value="aggressive">Aggressive (force .js/.json/.xml)</option>
      </select>
  ...
  <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
    <div style="font-size:11px;color:#666;margin-right:auto" id="scan-info">Whitelist ext + MIME heuristics (aggressive forces .js/.json/.xml)</div>
  ...
  // shouldFetchResource: improved logic with aggressive fallback for common types
  async function shouldFetchResource(url, mode, preferSameOrigin){
    try{
      ...
      // aggressive mode: always allow local text-like ext (.js/.json/.xml/.html)
      if(mode === 'aggressive'){
        const p = parsed.pathname.toLowerCase();
        if(p.endsWith('.js') || p.endsWith('.json') || p.endsWith('.xml') || p.endsWith('.html') || p.endsWith('.htm')) return true;
      }

      // prefer GET for .js chunk because many keys are there
      const pth = parsed.pathname.toLowerCase();
      if(pth.endsWith('.js') || pth.match(/\.chunk\.js$/) || pth.endsWith('.json') || pth.endsWith('.xml')) {
        // only allow if same-origin (we don't want cross-origin GETs)
        return parsed.origin === location.origin;
      }

      // if cached decision exists
      const abs = parsed.href;
      if(fetchCache.has(abs)) return !!fetchCache.get(abs);

      // smart mode: try HEAD for same-origin to check content-type
      if(parsed.origin === location.origin){
        try{
          const r = await fetch(abs, { method: 'HEAD', credentials: 'same-origin', signal: AbortSignal.timeout(3000) });
          const contentType = r.headers.get('content-type') || '';
          const mime = contentType.split(';')[0].toLowerCase();
          if(mime && Array.from(TEXT_MIME).some(t => mime.includes(t))) return true;
          // if no content-type or unknown, allow GET fallback for check
          if(!mime) return true;
          // otherwise, not text-like
          return false;
        }catch(e){
          registerFetchError(abs, `HEAD_ERR:${String(e && e.message ? e.message : e)}`);
          // fallback to GET for same-origin when HEAD fails
          return parsed.origin === location.origin;
        }
      } else {
        // cross-origin: skip fetching entire body
        return false;
      }
    }catch(e){
      return false;
  ...
  // fetchTextQuiet with fallback and not prematurely caching null for HEAD fail
  async function fetchTextQuiet(url){
    ...
    try{
      ...
      // Allow fetching even if content-type is missing/odd, particularly for .js/.json/.xml
      // But if type exists and clearly binary/not in text mime and not JS/JSON/XML, skip.
      ...
      if(ctype && !Array.from(TEXT_MIME).some(m => ctype.includes(m)) && !looksLikeJs && !looksLikeJson && !looksLikeXml){
        fetchCache.set(abs, null);
        skipped.add(abs);
        incSkipped();
        return null;
      ...
      // store even if empty to avoid infinite retries
  ...
  function contextHasKeywords(text, index, keywords, radius=60){
    ...
    for(const k of keywords) if(snippet.includes(k)) return true;
    return false;
  }
  ...
  function scanSensitive(text){
    ...
    for(const m of text.matchAll(DET.pwdParam) || []) { if(m[1]) res.push({type:'credential_like', value:m[1], contextIndex: m.index}); }
    ...
    for(const m of text.matchAll(DET.appKey) || []) { if(m[1]) res.push({type:'app_key', value:m[1], contextIndex: m.index}); }
  ...
  // progress
  let totalResources = 0, processedResources = 0, skippedResources = 0, stopRequested = false;
  ...
  function addFound(url, source='detected'){
    try{
      const abs = new URL(url, location.href).href;
      if(!found.has(abs)){
        found.set(abs, { params: new Set(), sensitive: [], misconfig: [], source, local: isLocal(abs), apiLike: isApiLike(abs), firebaseMeta: {}, fetchErrors: fetchErrors.get(abs) || [] });
      } else {
  ...
  async function process(url, depth=0, source='root', mode='smart', preferSameOrigin=true){
    ...
    // check extension presence/whitelist
    try{
      ...
      if(lastDot > -1){
        ...
        if(!SCAN_EXTENSIONS.has(ext) && mode === 'ext-only'){
          skipped.add(abs); incSkipped(); return;
        }
      } else {
        if(mode === 'ext-only'){ skipped.add(abs); incSkipped(); return; }
      }
    }catch(e){}
      ...
      if(cachedText){
        if(detectKeysCheckbox.checked){
          ...
          for(const s of sensitive){
            ...
            if(['git_exposed','env_filename','htpasswd','config_file_hint','possible_leak'].includes(s.type)) found.get(abs).misconfig.push(Object.assign({}, s, {context: snippet}));
            else {
    ...
    if(!shouldFetch){
      skipped.add(abs); incSkipped(); fetchCache.set(abs, null); return;
    }
  ...
  panel.querySelector('#copy-all').onclick = async ()=>{
    ...
    if(includeRaw){ if(!confirm('You have selected to include raw secrets. This will copy sensitive data to your clipboard. Proceed?')) return; }
    const lines = [];
    for(const r of rows){
  ...
  panel.querySelector('#export-txt').onclick = ()=>{
    ...
    const rows = buildList();
    const includeRaw = includeRawCheckbox.checked;
    if(includeRaw){ if(!confirm('You have selected to include raw secrets in the export file. This will write sensitive data to disk. Proceed?')) return; }
    const lines = [];
    function pushItem(r){
    ...
    } else { for(const r of rows) pushItem(r); }
    const blob = new Blob([lines.join('\n')], {type:'text/plain'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'endpoints-v4.2.txt'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  };
  ...
  panel.querySelector('#export-json').onclick = ()=>{
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'result-v4.2.json'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  };

  panel.querySelector('#close-panel').onclick = ()=> panel.remove();

  // helper isParamBearing
