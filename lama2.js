...
(async function(){
  ...
  // MEDIA IGNORE: skip common image/audio/video/media extensions to speed scan
  const MEDIA_EXT_REGEX = /\.(png|jpe?g|gif|svg|ico|bmp|webp|avif|mp4|webm|ogg|mp3|wav|m4a)(?:[?#].*)?$/i;
  ...
  function extractCandidatesFromText(text){
    ...
    for(const m of text.matchAll(re1)) {
      ...
      if(MEDIA_EXT_REGEX.test(candidate)) continue;
      s.add(candidate);
    }
    const re2 = /`([^`]{1,800})`/g;
    for(const m of text.matchAll(re2)) {
      const cleaned = m[1].replace(/\$\{[^}]+\}/g,'{param}');
      if(!MEDIA_EXT_REGEX.test(cleaned)) s.add(cleaned);
    }
    const ajaxRe = /(?:fetch|axios\.(?:get|post|put|delete|patch)|open)\s*\(\s*['"`]((?:https?:\/\/|\/|\.\.\/|\.\/)[^'"`]{1,800})['"`]/g;
    for(const m of text.matchAll(ajaxRe)) {
      const candidate = m[1].replace(/\$\{[^}]+\}/g,'{param}');
      if(MEDIA_EXT_REGEX.test(candidate)) continue;
      s.add(candidate);
    }
    return Array.from(s);
  ...
  async function process(url, depth=0, source='root'){
    ...
    // Skip media resources entirely (no adding, no processing)
    if(MEDIA_EXT_REGEX.test(abs)) return;
    ...
    for(const c of cands){
      try{
        // Skip candidates that are media before attempting to resolve/process
        if(MEDIA_EXT_REGEX.test(c)) continue;
        const resolved = new URL(c, abs).href;
        if(MEDIA_EXT_REGEX.test(resolved)) continue;
  ...
  // remove media-like resources early for efficiency
  for(const r of Array.from(resources)){
    try{
      // if the resource is a data: URI or blob skip media check
      if(String(r).startsWith('data:') || String(r).startsWith('blob:')) continue;
      if(MEDIA_EXT_REGEX.test(r)) resources.delete(r);
    }catch(e){}
  }
