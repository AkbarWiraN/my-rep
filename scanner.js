javascript:(async function(){
  ...
  <div id="scan-log" style="margin-top:8px">Collecting resources...</div>
  <hr>
  <div id="results" style="max-height:40vh;overflow:auto;padding-left:8px"></div>`;
  document.body.appendChild(panel);

  const logDiv=panel.querySelector("#scan-log"),
  ...
  function hostExcluded(host){ return excludedHosts.some(r=>r.test(host)); }
  function isLocal(u){ try{const url=new URL(u,location.href); return url.origin===location.origin&&!hostExcluded(url.host);}catch(e){return false;} }
  ...
  async function fetchText(url){
    try{
      ...
      if(!r.ok){ log(`Failed ${url} -> ${r.status}`); return null; }
      return await r.text();
  ...
  function addFound(abs,meta={}){
    try{
      ...
      if(!found.has(u)) found.set(u,{
        params:new Set(),keys:new Set(),source:meta.source||'detected',
  ...
  async function processUrl(u,depth=0,source='root'){
    ...
    if(!isLocal(abs)){
      log(`Skipped fetch (external or excluded): ${abs}`);
      return;
    }
  ...
  log(`Resource candidates: ${resources.size}`);
  for(const r of resources) await processUrl(r,0,'resource');

  function buildList(){
    const list=[];
    for(const [u,meta] of found.entries())
      list.push({url:u,params:[...meta.params],keys:[...meta.keys],
                 source:meta.source,local:meta.local,apiLike:meta.apiLike});
    ...
    const html=rows.map(it=>`
      <div style="padding:6px;border-bottom:1px solid #eee">
        ...
        <div style="font-size:12px;color:#333">
          ...
          ${it.keys.length?`<span style="color:crimson">keys: ${it.keys.map(()=> '[REDACTED]').join(', ')}</span>`:''}
  ...
  panel.querySelector("#copy-all").onclick=async ()=>{
    ...
    const text=rows.map(r=>
      `${r.url}${r.params.length?` | params: ${r.params.join(',')}`:''}${r.keys.length?` | keys: ${r.keys.join(',')}`:''} | src: ${r.source}`
    ).join("\n");
    try{
